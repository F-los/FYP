...
function NodeDetail() {
  ...
  useEffect(() => {
    const checkboxContainer = document.getElementById('controls');
  ...
  const order = JSON.parse(orderString).map(group =>
  ...
  // const order = orderString.split(';').map(group => group.split(',').map(item => item.trim()));
  ...
  function plotgraph(selectedNodeId) {
    const container = document.getElementById("second-sigma-container");
    if (!container) {
      console.error("Container #second-sigma-container not found");
      return;
    }

    const graph2 = new MultiGraph();
    // Here you would add nodes and edges specific to the selected recipe
    graph2.addNode(selectedNodeId, {
      x: Math.random(),
      y: Math.random(),
      size: 10,
      color: 'rgb(255,0,0)',
    ...
    var renderer2 = new Sigma(graph2, container, {
      minCameraRatio: 0.1,
      maxCameraRatio: 100,
    });
  }
