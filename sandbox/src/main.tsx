  ...
  fetch("./final.gexf")
    ...
    .then((gexf) => {
      ...
      function setupSecondGraph() {
        fetch("./modified_final.gexf")
          .then(res => res.text())
          .then(gexf => {
            const second_container = document.getElementById("second-sigma-container");
            if (!second_container) return;
            const graph2 = parse(Graph, gexf);
            var renderer2 = new Sigma(graph2, second_container, {
              minCameraRatio: 0.1,
              maxCameraRatio: 100,
            });
            var camera = renderer.getCamera();

            function updateSearchSuggestionsSecondGraph(query) {
              const searchInput = document.getElementById("searchInput"); // Ensure you have a corresponding input for the second graph
              ...
              graph2.nodes().forEach((node) => {
                  const label = graph2.getNodeAttribute(node, "label").toLowerCase();
                  if (label.includes(queryTrimmed)) {
                      suggestions.push(node);
                  }
              });
          
              if (suggestions.length > 0) {
                  // Select the first suggestion and focus the camera on it
                  const firstSuggestion = suggestions[0];
                  const nodePosition = renderer2.getNodeDisplayData(firstSuggestion);
                  renderer2.getCamera().animate(nodePosition, { duration: 100 });
                  // Additional UI updates or alerts can go here
                  console.log("Suggestions found: ", suggestions.map(n => graph2.getNodeAttribute(n, "label")));
              } else {
          ...
          findButton.addEventListener("click", function () {
            ...
            updateSearchSuggestionsSecondGraph(query);
          });
          
        //   document.getElementById("findButton2").addEventListener("click", () => {
